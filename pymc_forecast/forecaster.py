    ...
    def draw_posterior(
        ...
        *,
        ...
        batch_size
            ...
            :class:`Forecaster`), draw at most this many at once and concatenate
            the host-backed xarray chunks. This bounds the peak allocation made
            by ``pm.Approximation.sample`` on wide panels. ``None`` keeps the
            single-shot path. Backends whose posterior is already materialized
            (HMC and Pathfinder) thin it once and do not need this memory knob.
        """
...
class Forecaster(BaseForecaster):
    ...
    backend
        ``None`` or ``"pytensor"`` uses ``pm.fit``. ``"jax"`` runs
        mean-field ADVI and Adam as one JAX ``lax.scan`` on the selected
        accelerator, while retaining PyMC's ordinary approximation object for
        posterior sampling. The optional ``jax`` extra is required.
    num_steps
